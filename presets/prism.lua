...
local function draw_lyric(l, f, ctx, time)
  ...
  local width = math.max(f:measure(text).width, 1.0)
  ...
  l:text(text, { anchor = "top-left" }, function(i, g)
    ...
    -- Base position keeps reading order; the sine gives it vertical life.
    local base_x = f.width * 0.06 + t * f.width * 0.88
    local base_y = f.height * 0.5 + math.sin(t * 4.0 + time * 0.15 + seed_line) * f.height * 0.16

    local px = base_x + math.sin(time * 0.33 + seed * TAU) * f.width * 0.018
    local py = base_y + math.cos(time * 0.27 + seed * TAU) * f.height * 0.035
    local rotation = math.sin(time * 0.30 + seed * TAU) * 14.0
    local scale = 1.0 + math.sin(time * 0.21 + seed * TAU) * 0.28
    local hue = t * 0.8 + time * 0.04 + seed * 0.15
...
return {
  ...
  on_frame = function(f, ctx)
    ...
    f:layer({ post = "bloom", uniforms = { amount = 0.55, threshold = 0.30 } }, function(l)
