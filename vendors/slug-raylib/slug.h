...
inline constexpr int kBandSplits = 16;

inline constexpr const char kVertShaderCode[] =
  "#version 430\n"
  "struct VertexData {\n"
  "  vec4 pos;\n"
  "  vec4 tex;\n"
...
inline constexpr const char kFragShaderCode[] =
  "#version 430\n"
...
struct SlugFont {
    ...
    Glyph(stbtt_fontinfo& fontInfo, const int codepoint) {
      ...
      for (int i = 0; i < nCurves; ++i) {
        ...
        if (v.type == STBTT_vmove) {
          ...
          // CFF/OTF outlines (e.g. Noto Sans CJK) are cubic Beziers. The Slug
          // shader only solves quadratics, so split the cubic at t = 0.5 with
          // de Casteljau and approximate each half by a quadratic that shares
          // its endpoints and midpoint.
          const float x0 = static_cast<float>(lastX);
          const float y0 = static_cast<float>(lastY);
          const float x1 = static_cast<float>(v.cx);
          const float y1 = static_cast<float>(v.cy);
          const float x2 = static_cast<float>(v.cx1);
          const float y2 = static_cast<float>(v.cy1);
          const float x3 = static_cast<float>(v.x);
          const float y3 = static_cast<float>(v.y);

          const float a0x = (x0 + x1) * 0.5f, a0y = (y0 + y1) * 0.5f;
          const float a1x = (x1 + x2) * 0.5f, a1y = (y1 + y2) * 0.5f;
          const float a2x = (x2 + x3) * 0.5f, a2y = (y2 + y3) * 0.5f;
          const float b0x = (a0x + a1x) * 0.5f, b0y = (a0y + a1y) * 0.5f;
          const float b1x = (a1x + a2x) * 0.5f, b1y = (a1y + a2y) * 0.5f;
          const float mx  = (b0x + b1x) * 0.5f, my  = (b0y + b1y) * 0.5f;

          BCurve first;
          first.from    = {x0, y0};
          first.control = {(3.0f * (a0x + b0x) - (x0 + mx)) * 0.25f,
                           (3.0f * (a0y + b0y) - (y0 + my)) * 0.25f};
          first.to      = {mx, my};
          curves.push_back(first);

          BCurve second;
          second.from    = {mx, my};
          second.control = {(3.0f * (b1x + a2x) - (mx + x3)) * 0.25f,
                            (3.0f * (b1y + a2y) - (my + y3)) * 0.25f};
          second.to      = {x3, y3};
      ...
      bandScaleV  = (bounds.width > 0.0f) ? (float) kBandSplits / bounds.width : 0.0f;
      bandOffsetV = -bounds.x * bandScaleV;

      bandScaleH  = (bounds.height > 0.0f) ? (float) kBandSplits / bounds.height : 0.0f;
  ...
  SlugFont(std::unique_ptr<uint8_t[]>&& fontData, int faceIndex, const int* codepoints, const int nCodepoints) {
    this->fontData = std::move(fontData);
    int fontOffset = stbtt_GetFontOffsetForIndex(this->fontData.get(), faceIndex);
    if (fontOffset < 0) fontOffset = 0;
    stbtt_InitFont(&this->fontInfo, this->fontData.get(), fontOffset);
  ...
  // Whether the loaded face actually provides an outline for this codepoint.
  bool HasGlyph(int codepoint) const {
    return stbtt_FindGlyphIndex(&fontInfo, codepoint) != 0;
  }
};
...
inline PSlugFont LoadFontSlugEx(const char* fontFileName, int faceIndex, const int* codepoints, int nCodepoints) {
  ...
  if (!fontDataUnmanaged || fontFileSize <= 0) return nullptr;
  ...
  return new (mem) SlugFont(std::move(fontData), faceIndex, uniqueCodepointsVec.data(), (int) uniqueCodepointsVec.size());
}

inline PSlugFont LoadFontSlug(const char* fontFileName, const int* codepoints, int nCodepoints) {
  return LoadFontSlugEx(fontFileName, 0, codepoints, nCodepoints);
}
